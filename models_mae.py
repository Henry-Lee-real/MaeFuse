...
#from util.loss import Fusionloss
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss(self, imgs, pred):
        ...
        #loss_t = Loss(recover ,imgs_vi ,imgs_ir)[0].to("cuda")
...
def mae_vit_base_patch16_dec512d8b_v(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs) # type: ignore
...
def mae_vit_large_patch16_decoder1(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16,
...
mae_decoder_1        = mae_vit_large_patch16_decoder1
